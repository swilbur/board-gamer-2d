...
var games = {};
games['51st State'] = require("./51st");
games['Checkers'] = require("./checkers");
games['Croatoan'] = require("./croatoan");
games['Cry Havoc'] = require("./cry_havoc");
games['Forbidden Stars'] = require("./forbidden_stars");
games['Scrabble'] = require("./scrabble");
games['Star Wars: Rebellion'] = require("./star_wars_rebellion");
games['Terra Mystica Automata'] = require("./tm_auto");
  ...
  fs.stat('serverdata.txt', function (err, stats) {
    ...
    fs.readFile('serverdata.txt', function (err, data) {
      ...
      for(var r in roomsById){
        roomsById[r].lastAccessed = new Date(roomsById[r].lastAccessed);
        checkForNoUsers(roomsById[r]);
      }
    });
  });

  process.on("SIGINT", function(){backupServerData(); console.log("SIGINT"); process.exit(1);});
  process.on("SIGTERM", function(){backupServerData(); console.log("SIGTERM"); process.exit(2);});
  process.on("uncaughtException", function(e){backupServerData(); console.log(e); process.exit(99);});
...
function checkForNoUsers(room) {
  for (var id in room.usersById) {
    if (room.usersById[id].socket) return; // nevermind. it's not empty.
  }
...
//  var role = findAvailableRole(room);
  console.log("b: " + usersById + " " + username + " " + usersById[username]);
          ...
          if (!usersById[username]) socket.sendText(JSON.stringify({cmd: "loginFailed", args:"Incorrect username or password."}));
            ...
            for (var r in roomsById) {
              if (username in roomsById[r].usersById){
                ...
                gameList.push({id: r,  gameName: roomsById[r].game.name, players: players, lastAccessed: roomsById[r].lastAccessed});
              }
            }
            gameList.sort(function(a,b){return b.lastAccessed - a.lastAccessed;}); // most recent games at the top
...
function backupServerData(){
  ...
  var roomsCleaned = {};
  for (var r in roomsById){
    roomsCleaned[r] = {id: roomsById[r].id, game: roomsById[r].game, usersById: {}, lastAccessed: roomsById[r].lastAccessed, changeHistory: roomsById[r].changeHistory, unusedTimeOutHandle: null};
  ...
  fs.writeFileSync('serverdata.txt', JSON.stringify({rooms: roomsCleaned, users: usersCleaned}));
}
